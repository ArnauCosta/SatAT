...
class ScalAT(problemName: String = "", workingpath: String = "working/") {
  ...
  def addAMOLog(x: List[Int]): Unit = {
    ...
    val n = x.length
    if (n <= 1) return // Res a fer si hi ha 0 o 1 literal

    val numAuxVars = Math.ceil(Math.log(n) / Math.log(2)).toInt // Nombre de variables binàries auxiliars
    val auxVars =  newVarArray(numAuxVars)// (1 to numAuxVars).map(i => s"b$i") // Variables auxiliars (podrien ser literals enters si ja estan definits)

    // Assignem un codi binari a cada literal
    val binaryCodes = x.indices.map(i => i.toBinaryString.reverse.padTo(numAuxVars, '0').reverse)

    // Per cada literal, afegim clàusules per garantir la seva correspondència amb el codi binari
    for ((xi, code) <- x.zip(binaryCodes)) {
      for ((b, bit) <- auxVars.zip(code)) {
        if (bit == '1') {
          addClause(List(-xi, b.toInt)) // xi -> b (negació)
        } else {
          addClause(List(-xi, -b.toInt)) // xi -> ¬b
        }
      }
    }

    // Assegurem que cap dues variables comparteixin el mateix codi binari
    for (i <- x.indices; j <- i + 1 until x.length) {
      val clauses = auxVars.indices.map(k => {
        val litI = if (binaryCodes(i)(k) == '1') auxVars(k).toInt else -auxVars(k).toInt
        val litJ = if (binaryCodes(j)(k) == '1') auxVars(k).toInt else -auxVars(k).toInt
        List(-litI, -litJ) // Assegurar que els bits no coincideixen
      })
  ...
  def addEK(x: List[Int], K: Int): Unit = {
    ...
    val sorted = newVarArray(x.length)
    addSorter(x, sorted.toList)

    if (K > 0) addClause(List(sorted(K - 1)))
    if (K < sorted.length) addClause(List(-sorted(K)))
  }
  ...
  def addALK(x: List[Int], K: Int): Unit = {
    ...
    val sorted = newVarArray(x.length)
    addSorter(x, sorted.toList)
    println("Sorted to list: ")
    println(sorted.toList)

    println("Taking k: ")
    println(sorted.take(K).toList)

    for (v <- sorted.take(K).toList) {
      addClause(List(v))
    }
  }
  ...
  def addAMK(x: List[Int], K: Int): Unit = {
    ...
    for (i <- K until sorted.length) {
      addClause(List(-sorted(i)))
    }
  }
